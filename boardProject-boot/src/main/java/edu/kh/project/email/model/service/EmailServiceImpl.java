...
public class EmailServiceImpl implements EmailService {
	...
	private final SpringTemplateEngine templeteEngine;
	// SpringTemplateEngine : 타임리프를 이용해서 html 코드 적다 말음..
	...
	public String sendEmail(String htmlName, String email) {
		...
		// 메일 발송을 도와주는 Helper 클래스(ㅠㅏ일첨부, 템플릿 설정 등 쉽게 처리)
		try {
			MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
			// - mimeMessage : MimeMessage 객체로, 이메일 메시지의 내용을 담고있음
			// 				   이메일의 본문, 제목, 발송자 정보, 수신자 정보 등등 포함
	...
	private String loadHtml(String authKey, String htmlName) {
		...
		return templeteEngine.process("email/" + htmlName, context);
		// templete/email/signup.html
	}

	// 인증키와 이메일을 DB 에 저장하는 메서드
	@Transactional(rollbackFor=Exception.class) // 메소드 레벨에서도 이용 가능(해당 메서드에서만 트랜잭션 커밋/롤백)
