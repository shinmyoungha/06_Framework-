...
public class EditBoardController {
	...
	public String boardInsert(@PathVariable("boardCode") int boardCode, @ModelAttribute Board inputBoard,
		...
		/*
		 * List<MultipartFile> images - 5개 모두 업로드 O -> List안에 0~4번 인덱스에 파일 저장됨 - 5개 모두
		 * 업로드 X -> List안에 0~4번 인덱스에 파일 저장 X - 2번 인덱스만 업로드 -> 2번 인덱스만 파일 저장, 0/1/3/4번
		 * 인덱스는 저장 X
		 * 
		 * [문제점] - 파일이 선택되지 않은 input 태그도 제출되고 있음 (제출은 되어있는데 데이터는 없음)
		 * 
		 * -> 파일 선택이 안된 input 태그 값을 서버에 저장하려고 하면 오류 발생함!
		 * 
		 * [해결방법] - 무작정 서버에 저장 X -> List의 각 인덱스에 들어있는 MultipartFile에 실제로 제출된 파일의 데이터가
		 * 있는지 확인하는 로직을 추가 구성
		 * 
